...
def prepare_data():
    df = get_data()
    df["coredata_openaccess"] = pd.to_numeric(
        df["coredata_openaccess"], errors="coerce"
    )
    ...
    open_access_summary = (
        df.groupby(["publishername", "openaccess"]).size().unstack(fill_value=0)
    )
    ...
    open_access_summary["percentage_range"] = open_access_summary[
        "open_access_percentage"
    ].apply(get_range_label)

    range_summary = (
        open_access_summary.groupby("percentage_range")
        .agg(
            {"publishername": lambda x: ", ".join(x), "open_access_percentage": "count"}
        )
        .reset_index()
    )

    range_order = [f"{i}-{i+10}%" for i in range(0, 100, 10)]
    range_summary["range_order"] = pd.Categorical(
        range_summary["percentage_range"], categories=range_order, ordered=True
    )
